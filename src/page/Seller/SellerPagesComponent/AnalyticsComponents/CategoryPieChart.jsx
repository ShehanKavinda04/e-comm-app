    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-gray-800 text-white text-xs rounded p-2 shadow-lg">
          <p className="label">{`${payload[0].name}: Rs.${parseInt(payload[0].value).toLocaleString()}`}</p>
        </div>
      );
    }
    return null;
    ...
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col items-center justify-center relative">
      ...
      <div style={{ width: '100%', height: 250, position: 'relative' }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none pb-8">
          <span className="text-xl font-bold text-gray-900">Rs.{totalRevenue ? totalRevenue.toLocaleString() : '0'}</span>
          <span className="text-gray-500 text-xs">Total</span>
        </div>
      </div>

      <div className="flex flex-wrap justify-center gap-x-6 gap-y-2 w-full mt-[-20px]">
        {data && data.map((item, index) => (
          <div key={index} className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-600">
              {item.name} ({totalRevenue > 0 ? ((item.value / totalRevenue) * 100).toFixed(0) : 0}%)
            </span>
          </div>
        ))}
      </div>
